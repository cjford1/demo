...
lab2.factory('processMove', function() {
  return function() {
    self.checkWins =  function(player1_choice, player2_choice) {
      ...
      var player1 = this.getChoiceAsNumber(a);
      var player2 = this.getChoiceAsNumber(b);
      /* return 0 if tie */
      if (player1 == player2) return 0;

      /* return 1 if player 1 won the current round */
      if (player1 == 0 && player2 == 2) return 1;
      ...
      if (player1 == 2 && player2 == 0) return 2;
      if (player1 == 0 && player2 == 1) return 2;
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $scope.onNext = function (choice) {
    ...
    /* use if statement that directly updates $stateParams */
    if ($scope.processMove.checkWins($stateParams.player1_choice, choice) == 1 ) {
      $stateParams.player1_wins++;
    }
    /* TODO: Use processMove factory instance to check if player2 has a win */
    /* use if statement that directly updates $stateParams */
    if ($scope.processMove.checkWins($stateParams.player1_choice, choice) == 2 ) {
